...
import { handleAutomationAction } from "@/lib/automation/actions";
...
export async function POST(req: NextRequest) {
  ...
  try {
    // AI follow-up engine + human handoff (lib/automation/actions.ts).
    const automation = await handleAutomationAction(action, payload, db, creds);
      ...
      case "send_message": {
        ...
        if (channel === "whatsapp") {
          ...
          providerId = await sendDirectMessage({
            pageId: channel === "instagram" ? creds.fbPageId : creds.fbPageId,
